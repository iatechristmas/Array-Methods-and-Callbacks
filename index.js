...
function getAllWinners(data, getYears, getWinners) {
    ...
    return newWinners.map((item, index) => `In ${newYears[index]}, ${item} won the world cup!`);
};
...
function getGoals(data) {
    const cupWins = getFinals(data);
    const taArray = cupWins.map(function (item) {
        return { teamname: item["Home Team Name"], goals: item["Home Team Goals"] }
    })
    return taArray
